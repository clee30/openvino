        ...
        if (is_second && params.mode != ScatterUpdateReduction::NONE) {
            switch (rank) {
                ...
                case 6:
                    ...
                    dispatchData.lws = {indices.X().v * indices.Y().v, indices.Z().v * indices.W().v, indices.Feature().v * indices.Batch().v };
                    break;
                default:
...
void ScatterElementsUpdateKernelRef::GetUpdateDispatchDataFunc(KernelData& kd) const {
    kd.update_dispatch_data_func = [this](const Params& params, KernelData& kd) {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            //const auto& indices = prim_params.inputs[2];
            const auto& output = prim_params.outputs[0];
            if (i == 1) {
                auto buffer_size = output.PhysicalSizeInBytes() * 2;
                ScalarDescriptor local_memory_size_scalar;
                local_memory_size_scalar.t = ScalarDescriptor::Types::UINT32;
                local_memory_size_scalar.v.u32 = buffer_size;
                kd.kernels[i].params.scalars.clear();
                kd.kernels[i].params.scalars.push_back(local_memory_size_scalar);
...
KernelsData ScatterElementsUpdateKernelRef::GetKernelsData(const Params& params) const {
    ...
    KernelData kd = KernelData::Default<scatter_elements_update_params>(params, 3);
