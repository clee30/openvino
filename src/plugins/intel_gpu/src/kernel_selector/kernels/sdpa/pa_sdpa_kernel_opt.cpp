...
void PagedAttentionSDPAKernelOpt::GetUpdateDispatchDataFunc(KernelData& kd) const {
    kd.update_dispatch_data_func = [](const Params& params, KernelData& kd) {
        ...
        if (prim_params.stage == PagedAttentionStage::PREFILL) {
            partition_size = SDPAKernelOpt::get_seq_len_partition_size(params, prim_params.conf.k_head_size, 1);
        } else {
            partition_size = seq_len_partition_size;
        }
        ...
        auto tmp_out_elements_count = total_tokens * prim_params.conf.heads_num * prim_params.conf.k_head_size * num_of_partitions;
