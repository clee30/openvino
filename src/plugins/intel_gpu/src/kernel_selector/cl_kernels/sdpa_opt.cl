...
//    if (get_global_linear_id()== 0)
//        printf("sdpa_opt first token, SOURCE_SEQ_LEN=%d, TARGET_SEQ_LEN=%d\n", SOURCE_SEQ_LEN, TARGET_SEQ_LEN);
...
#endif
                __attribute__((opencl_unroll_hint(1)))
                for (uint head_idx_index = 0; head_idx_index < K_HEAD_SIZE; head_idx_index += SUBGROUP_SIZE) {
...
#endif
//        if (output_offset == 64)
//            printf("(%d %d %d) sgid=%d, SUBGROUPS_PER_WG=%d, SG_SCALE_FACTOR=%d\n", get_global_id(0), get_global_id(1), get_global_id(2), sgid, SUBGROUPS_PER_WG, SG_SCALE_FACTOR);
//            printf("id=(%d %d %d) output_offset=%d, output_pitch=%d\n", get_global_id(0), get_global_id(1), get_global_id(2), output_offset, output_pitch); 


        if (TARGET_SEQ_LEN_BLOCK_SIZE > seq_idx_end) {
            for (uint seq_idx = 0; seq_idx < seq_idx_end; seq_idx++) {
                output_acc[seq_idx] /= slm_exp_sum_prev[seq_idx];
//                if ((get_global_id(0) == 1) && (get_global_id(1) == 0)  && (get_global_id(2) == 8))
//                    printf("id=%d, output_offset=%d, output_pitch=%d, output_acc=%f\n", get_global_id(2), output_offset, output_pitch, (float)output_acc[seq_idx]);
