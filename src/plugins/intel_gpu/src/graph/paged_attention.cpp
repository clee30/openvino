...
std::vector<layout> paged_attention_inst::calc_output_layouts(paged_attention_node const& /*node*/, kernel_impl_params const& impl_param) {
    ...
    auto k_layout = impl_param.get_input_layout(1);
    auto v_layout = impl_param.get_input_layout(2);
    auto data_layout = q_layout;
    if (v_layout.is_static()) {
//          static_cast<long int>(v_layout.get_shape()[0]) != static_cast<long int>(k_layout.get_shape()[0])) {
        ShapeType v_shape = v_layout.get_shape();
        auto v_shape_size = v_shape.size();
        ShapeType test_shape = data_layout.get<ShapeType>();
        test_shape[v_shape_size - 1] = v_shape[v_shape_size - 1];
        data_layout = layout{test_shape, data_layout.data_type, data_layout.format};
    }
